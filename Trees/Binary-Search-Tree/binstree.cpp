...
void BST::DeleteVertex(el_t E)
{
  ...
  { cout << "... deleting the root with just one child" << endl; 
    ...
    if (Root->Left == NULL && Root->Right != NULL) // if it has only the right subtree
    { 
   ...
   P->Left = NULL;   
     ...
     {    Vertex* C = V->Left; // C is the left child
	  cout << "removing a vertex with just the left child" << endl;
          if (P->Left == V) 
        P->Left = C; // Bypass V by setting P's left child to C
    else 
        P->Right = C; // Bypass V by setting P's right child to C
