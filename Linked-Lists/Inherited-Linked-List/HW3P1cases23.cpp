...
{ 
  ...
  // 6. insert the 9th  (element 12) error (out of range)
    ...
    catch(llist::OutOfRange) {cout << "Error: You are out of range" << endl;}  // do not exit
  ...
  // 12. delete Ith I==5 . error (out of range)
    ...
    try {L.deleteIth(5, x);}
    catch(llist::OutOfRange){cout << "Error: You are out of range" << endl;}
      ...
      while(!L.isEmpty())
{
  ...
  try { L.deleteFront(x);}
  catch (llist::Underflow){cout << "Error: You caused an Underflow" << endl;}
...
void caseThree()
{
 ...
 cout << "-- After copytest --- " << endl;
  ...
  cout << "Step " << counter << endl; counter++;
  L2.addRear(7); L2.addRear(8);
  L2.addRear(9); L2.addRear(10);
